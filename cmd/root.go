...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&config, "config", "", "config file (default is $HOME/.p.yaml or $XDG_CONFIG_HOME/p/config.yaml)")
	rootCmd.PersistentFlags().StringVar(&ProjectsPath, "projects_path", "", "The root path of all projects")
